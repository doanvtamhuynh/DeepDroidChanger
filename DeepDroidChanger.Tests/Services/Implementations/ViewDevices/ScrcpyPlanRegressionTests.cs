...
public sealed class ScrcpyPlanRegressionTests
{
    ...
    [TestMethod]
    public void FfmpegSendDrainFlow_EagainWithoutProgressFailsInsteadOfSpinning()
    {
        int sendCount = 0;
        ...
        InvalidOperationException exception = Assert.ThrowsExactly<InvalidOperationException>(
            () => FfmpegSendDrainFlow.SendPacketAndDrain(
                ...
                () => 0,
                result => result == -11,
                result => new InvalidOperationException($"fatal {result}")));

        StringAssert.Contains(exception.Message, "no frames");
        Assert.AreEqual(1, sendCount);
    }

    [TestMethod]
    public void FfmpegSendDrainFlow_FatalSendResultUsesFatalException()
    {
        int drainCount = 0;
        ...
        StringAssert.Contains(exception.Message, "fatal -123");
        Assert.AreEqual(0, drainCount);
    }

    [TestMethod]
    public void FfmpegSendDrainFlow_SuccessPerformsOneFinalDrain()
    {
        int drainCount = 0;
        ...
        Assert.AreEqual(1, drainCount);
    }

    [TestMethod]
    public void NonOomSubscriberException_IsLoggedAndOtherSubscriberCanContinue()
    {
        InvalidOperationException expected = new("subscriber failed");
        List<Exception> logged = [];
        int continued = 0;

        VideoStreamDecoder.InvokeSubscriberSafely(
            () => throw expected,
            logged.Add);
        VideoStreamDecoder.InvokeSubscriberSafely(
            () => continued++,
        ...
        Assert.AreSame(expected, logged[0]);
        Assert.AreEqual(1, continued);
    }

    [TestMethod]
    public void OutOfMemorySubscriberException_IsRethrown()
    {
        OutOfMemoryException expected = new("decoder memory exhausted");
        List<Exception> logged = [];
    ...
    [TestMethod]
    public void ScrcpyDisplay_NormalizesSystemKeyToPhysicalSystemKey()
    {
        Assert.AreEqual(
            Key.H,
            ScrcpyDisplay.NormalizePhysicalKey(Key.System, Key.H));
        Assert.AreEqual(
            Key.F,
            ScrcpyDisplay.NormalizePhysicalKey(Key.F, Key.F));
    }
