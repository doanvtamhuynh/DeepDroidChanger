...
internal sealed class AccountStoreService : IAccountStoreService
{
    ...
    public AccountStoreService(
        IOptions<AccountStoreOptions> options,
        ILogger<AccountStoreService> logger)
    {
        _accountPath = Path.GetFullPath(options.Value.AccountFilePath);
        _accountDirectory = Path.GetDirectoryName(_accountPath)
            ?? throw new ArgumentException("Account path must include a directory.", nameof(options));
        _logger = logger;
    }
    ...
    public async Task<AccountLoginRequest?> LoadSavedLoginAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            AccountSettings? account = await JsonSerializer
                .DeserializeAsync<AccountSettings>(stream, JsonOptions, cancellationToken)
                .ConfigureAwait(false);
            if (account is not { RememberAccount: true })
                return null;

            string password = Unprotect(account.ProtectedPassword, account.Entropy);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await WriteAllTextAtomicallyAsync(_accountPath, json, cancellationToken)
                .ConfigureAwait(false);
        }
        finally
        {
    ...
    private static string Unprotect(string protectedValue, string entropyValue)
    {
        ...
        byte[] unprotectedData = ProtectedData.Unprotect(data, entropy, DataProtectionScope.CurrentUser);
